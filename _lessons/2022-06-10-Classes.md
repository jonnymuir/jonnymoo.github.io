...
We can also have "attributes". Things that belong the a LaserFrog. One way to do this is to just set it. Here we introduce you to another thing about programming - Gobblydegook you just have to learn. But don't worry, you'll best learn it by just looking at what other people do and by usually getting it wrong. This is not only ok, it is very normal.

We can see some gobblydegook at play below. Specifically, every in method the word "self" is always the first parameter. You use it to refer to the "instance" of the thing (in our case an instance of a LaserFrog).
...
Ok. Above I've decided that an attribute of a LaserFrog is to have some LaserCharge, lets do some more interesting things with it though. Every time we fire a laser, lets reduce it. If we run out of laser - sad face.
